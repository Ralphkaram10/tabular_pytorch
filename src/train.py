...
def train_model(config):
    ...
    init_input_dict = {'X': train_val_test_data_dict['X_train'], 'y': train_val_test_data_dict['y_train']}
    init_output_dict = init(init_input_dict)
    # Training loop
    pbar=tqdm(range(config["num_epochs"]))
    for epoch in pbar:
        pbar.set_description('training')
        train_one_epoch_input_dict={**init_output_dict,
                                    'train_loader':train_val_test_data_dict['train_loader']
                                    }
        train_one_epoch_output_dict=train_one_epoch(train_one_epoch_input_dict)
        eval_one_epoch_input_dict={**train_one_epoch_output_dict,
                                    'val_loader':train_val_test_data_dict['val_loader']
                                    }
...
def load_train_val_test_data(config):
    ...
    train_dataset = CustomDataset(
        input_table_path=config["train_input_table_path"],
        target_table_path=config["train_target_table_path"])
    val_dataset = CustomDataset(
        input_table_path=config["val_input_table_path"],
        target_table_path=config["val_target_table_path"])
...
def train_one_epoch(input_dict):
    ...
    output_dict={'train_loss':train_loss,'model':input_dict['model'],'criterion':input_dict['criterion']}
    return output_dict

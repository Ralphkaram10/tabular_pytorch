...
def split_dataframe(df, train_percent, val_percent, test_percent, random_state=None):
    """
    ...
    remaining_data, test_data = train_test_split(
        df, test_size=test_percent / 100, random_state=random_state
    )
    ...
    train_data, val_data = train_test_split(
        remaining_data, test_size=adjusted_val_percent / 100, random_state=random_state
    )
...
if __name__ == "__main__":
    ...
    train_df, val_df, test_df = split_dataframe(
        df, train_percent, val_percent, test_percent, random_state=random_state
    )
    ...
    train_output_path = os.path.join(
        output_dir_path, train_output_filename_with_extension
    )
    val_output_path = os.path.join(output_dir_path, val_output_filename_with_extension)
    test_output_path = os.path.join(
        output_dir_path, test_output_filename_with_extension
    )
